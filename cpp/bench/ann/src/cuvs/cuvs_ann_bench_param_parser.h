...
void parse_build_param(const nlohmann::json& conf, cuvs::neighbors::cagra::index_params& params)
{
  ...
  if (std::holds_alternative<std::monostate>(params.graph_build_params)) {
    if (!ivf_pq_build_conf.empty() || !ivf_pq_search_conf.empty()) {
      ...
      params.graph_build_params = cuvs::neighbors::graph_build_params::ace_params{};
    } else if (!build_search_conf.empty()) {
      params.graph_build_params = cuvs::neighbors::graph_build_params::iterative_search_params{};
    }
    ...
    [&](auto& arg) {
      ...
      if constexpr (std::is_same_v<U, cuvs::neighbors::graph_build_params::ivf_pq_params>) {
        ...
        if (build_search_conf.contains("algo")) {
          ...
          if (algo == "single_cta") {
          ...
          } else if (algo == "single_cta_ws") {
            arg.algo = cuvs::neighbors::cagra::search_algo::SINGLE_CTA_WS;
          } else if (algo == "single_cta_noop") {
            arg.algo = cuvs::neighbors::cagra::search_algo::SINGLE_CTA_NOOP;
            ...
            THROW("Invalid build_search algo: %s", algo.c_str());
          }
        }
                        ...
                        typename cuvs::bench::cuvs_cagra<T, IdxT>::search_param& param)
{
  ...
  if (conf.contains("algo")) {
    ...
    } else if (conf.at("algo") == "single_cta_ws") {
      param.p.algo = cuvs::neighbors::cagra::search_algo::SINGLE_CTA_WS;
    } else if (conf.at("algo") == "single_cta_noop") {
      param.p.algo = cuvs::neighbors::cagra::search_algo::SINGLE_CTA_NOOP;
